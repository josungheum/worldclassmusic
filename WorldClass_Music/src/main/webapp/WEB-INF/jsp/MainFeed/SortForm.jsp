...
<div class="modal-body">
	<div class="box-body" style="border-bottom-style: solid !important; border-bottom:2px; border-bottom-color: #d2d6de;">
		<div class="col-sm-12" style="margin-top: 28px; padding-left: 0px; padding-right: 0px;">
			<table id="mainFeedFormTable" class="table table-striped">
				<thead>
					<tr>
						<th>No.</th>
						<th>컨텐츠 명</th>
...
<input type="hidden" id="fileMediaType" />
...
function moveTable(){
	if(!(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent))){
		$('#mainFeedFormTable').tableDnD({
			onDragClass: "dragRow"
		});
	}
}

function MainFeedFormList(){
	var optObject = {};

	optObject.id = "#mainFeedFormTable";
	...
	optObject.arrColumnDefs = [
        {
            ...
            'render': function (data, type, full, meta) {
               	return dataTableRowIdx2(meta) + '<input type="hidden" id="mainFeedIdx" value="' + full.mainFeedIdx + '">';
            }
        },
        {
            'targets': 1,
...
function updateOrderSeq(){
	...
	var tableLength = $('#mainFeedFormTable').find('tbody').find('tr').length;
	
	$('#mainFeedFormTable').find('tbody').find('tr').each(function(e) {
		var mainFeedIdx = $(this).find('[id="mainFeedIdx"]').val();
		var orderSeq = tableLength - e;
		var param = {'mainFeedIdx': mainFeedIdx, 'orderSeq': orderSeq}

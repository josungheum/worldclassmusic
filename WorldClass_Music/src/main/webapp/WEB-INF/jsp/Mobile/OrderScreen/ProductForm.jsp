...
<form:form commandName="orderScreenVo" name="productform" id="productform" method="post" onsubmit="return false;">
	 <div class="modal-body">
		<div style="margin-top:4px; height:250px; overflow-y:scroll; overflow-x: hidden;" class="proton-demo" id="treeFind">
...
<script type="text/javaScript">
	$(document).ready(function () {
		$('.modal-dialog')[1].style.width = '300px';
		treeFind();
		addTableSelected();
	});
...
// 				innerHtml += '					<option value="STA0001">없음</option><option value="STA0002">신규</option><option value="STA0003">인기</option><option value="STA0004" >품절</option>';
				innerHtml += '				</select>';
				...
				innerHtml += '						</font><br>';
// 				if($(tr[i].children[4]).text() != $(tr[i].children[4]).find('input').val()){
...
// 				}else{
// 					innerHtml += '						<div style="font-size: 20px;width: 100%;font-weight: bold;text-align: right;">￦ '+autoComma($(tr[i].children[4]).text())+'</div>';
// 				}
...
// 				innerHtml += '		<button onclick="prodDelete()" style="width: 17px; height: 17px;" class="lEditorLayerlistItemDel" ot="lEditorLayerlistItemDel"></button>';
// 				innerHtml += '		<div class="card" style="width: 18rem;">';
// 				innerHtml += '			<img src="'+$(tr[i].children[1]).find('input').val()+'" class="card-img-top" alt="...">';
// 				innerHtml += '			<ul class="list-group list-group-flush">';
// 				innerHtml += '				<li class="list-group-item2" style="font-size: 5px;"><strong>'+$(tr[i].children[2]).text();
// 				innerHtml += '					</strong><br>';
// 				innerHtml += '					<font style="color:grey">'+$(tr[i].children[3]).text();
// 				innerHtml += '					</font><br>';
// 				if($(tr[i].children[4]).text() != $(tr[i].children[4]).find('input').val()){
// 					innerHtml += '					<strike>'+autoComma($(tr[i].children[4]).text())+' 원</strike>';
// 					innerHtml += '					<strong>'+autoComma($(tr[i].children[4]).find('input').val())+' 원</strong>';
// 				}else{
// 					innerHtml += '					<strong>'+autoComma($(tr[i].children[4]).text())+' 원</strong>';
// 				}
// 				innerHtml += '				</li>';
// 				innerHtml += '			</ul>';
// 				innerHtml += '		</div>';

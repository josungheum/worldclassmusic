...
<div class="box-body">
	<table id="userTable" class="table table-striped">
	    <thead>
	        <tr>
	            <th><input type="checkbox" name="ARRIDX_ALL" onclick="$.checkboxUtil.checkAll(this, 'checkboxArr');" id="checkboxArr_ALL" /></th>
	            <th>No</th>
	            <th>사용자명</th>
	            <th>아이디</th>
	            <th>생성일</th>
...
<script>
	...
	function userAdd() {
		$.sessionCheck();
		$.modalCommon.loadDataView('사용자 생성', '<c:url value="/UserManagement/UserFormNullTemp"/>', {brandIdx:$("#brandIdx").val(), francIdx:$("#francIdx").val()});
	}
	...
	function userList() {
		...
		optObject.arrColumnDefs = [
	        ...
	        },
	        {
	            ...
	            'render': function (data, type, full, meta)
	            {
	            	return '<button type="button" class="btn btn-sm btn-warning" onclick="PwdReset(\'' + full.id + '\', \'' + full.usersIdx + '\');" >비밀번호 변경</button>';
	            }
	        },{
		    	"targets": [4]
		       ,"className": "text-center"

(function ($) {
	...
	$.fn.ContentEditor.Init = function(settings, obj) {
		...
		$('<th>').appendTo(tr)
				 ...
				 .css('width', '135px')
				 .css('font-size', '20px');
		td = $('<td>').appendTo(tr).css('width', '235px');
		$('<input>').attr('type', 'text')
					.attr('ot', 'templateName')
					.attr('id', 'templateName')
					.attr('maxlength','50')
					.addClass('scpropTemplateName')
					.css('text-align', 'center')
					.css('width', '220px')
		...
		// 해상도 RESOLUTION_X
		$('<th>').appendTo(tr).text('해상도 :').css('font-size', '20px').css('width', '87px');
		td = $('<td>').appendTo(tr).attr('ot', 'resolutionX').css('width', '85px');
...
//					.attr('rowCnt', settings.rows)
		...
		// RESOLUTION_Y
		$('<th>').appendTo(tr).text('X ').css('font-size', '20px').css('width', '15px');
		td = $('<td>').appendTo(tr).attr('ot', 'resolutionY').css('width', '98px');
